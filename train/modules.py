import numpy as np
def postprocess(S,prefix,config_postprocess,specialWordFilter=True,dropPerson=True,
                maxNbSents=True,removeEndPunc=True,removeWords = True,
                removeSingleWord=True,transfer = True,sentEndcontent=True,
                removeDupulicate=True,dropSpecial=True,removeHighFreqWords=False,
                removeIncompletePunc=True,removeHighNumber=True):
    ...
    for s0 in S:
        ...
        if removeIncompletePunc:
            if not hasCompletePunc(s0[len(prefix):]):
                continue
        if removeHighNumber:
            if hasMuchNumbers(s0[len(prefix):]):
                continue
        if len(s0)>min_contenlen:
            ...
            if len(prefix)<=7 and len(s0) - len(prefix) > r*len(prefix):
                R.append(s0)
    return R
def hasMuchNumbers(s0,maxNb=3):
    syms = '0123456789'
    n = sum([int(t in syms) for t in s0])
    if n>=maxNb:
        return True
    else:
        return False
