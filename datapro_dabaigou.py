...
def build_files(data_path, tokenized_data_path, num_pieces, full_tokenizer, min_length, seq_len):
    ...
    for i in tqdm(range(num_pieces)):
        ...
        for subline in sublines:
            ...
            if len(tmp)<seq_len-1:
                tmp += [full_tokenizer.convert_tokens_to_ids('[PAD]')]*(seq_len-1-len(tmp))
            else:
                tmp = tmp[:seq_len-1]
            full_line = tmp + [full_tokenizer.convert_tokens_to_ids('[CLS]')]  # 文章之间添加CLS表示文章结束
...
def main():
    ...
    parser.add_argument('--seq_len', default=50, type=int, required=False)
    ...
    build_files(data_path=raw_data_path, tokenized_data_path=tokenized_data_path, num_pieces=num_pieces,
                full_tokenizer=full_tokenizer, min_length=min_length, seq_len=seq_len)
    print('files built')
